...
# chage the recursion limit due to FATAL error - unbalanced tree with multiple polytomies.
# from: https://github.com/nextstrain/augur/pull/307/files
sys.setrecursionlimit(int(10000))
os.environ["AUGUR_RECURSION_LIMIT"] = "10000"
...
rule filter_gapped_sequences:
    message:
        """
        Filtering to
          - percentage of Ns in a sequence {params.Ns}
        """
    input:
    ...
    shell:
        """
        python3 scripts/filter_gapped_sequences.py \
            --sequences {input.sequences} \
            --precent_N {params.per_N} \
            --output {output.sequences}
        """
